...
import { FiMessageSquare, FiMail, FiPhone, FiSend, FiChevronRight, FiHome, FiClock, FiCheck, FiArrowLeft, FiUser, FiArrowRight } from 'react-icons/fi';
...
export default function Messages() {
  ...
  const [tab, setTab] = useState(user?.role === 'buyer' ? 'sent' : 'inbox');

  const isAgent = user?.role === 'agent' || user?.role === 'seller' || user?.role === 'admin';

  useEffect(() => {
    if (isAgent && tab === 'inbox') loadAgentInquiries();
    else loadMyInquiries();
  }, [tab]);

  const loadAgentInquiries = async () => {
    setLoading(true);
    try {
      const endpoint = user.role === 'admin' ? '/inquiries' : '/inquiries/agent';
  ...
  const statusColors = { new: 'bg-blue-500', read: 'bg-gray-400', replied: 'bg-emerald-500', closed: 'bg-red-400' };
    ...
    <div className="pt-24 pb-16 min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="mb-6">
          ...
          <p className="text-gray-500 mt-1">{isAgent ? 'Inquiries from customers on your properties' : 'Your conversations with agents'}</p>
        </div>
        ...
        <div className="flex gap-1 mb-6 bg-white rounded-xl p-1 shadow-sm border border-gray-100 w-fit">
          {isAgent && (
            <button onClick={() => setTab('inbox')} className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${tab === 'inbox' ? 'bg-primary-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-50'}`}>
              Inbox ({inquiries.filter(i => i.status === 'new').length})
            </button>
          )}
          <button onClick={() => setTab('sent')} className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${tab === 'sent' ? 'bg-primary-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-50'}`}>
            {isAgent ? 'All Inquiries' : 'My Messages'}
          </button>
        </div>
                  ...
                  <button key={inq._id} onClick={() => selectInquiry(inq)}
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <span className={`w-2 h-2 rounded-full ${statusColors[inq.status] || 'bg-gray-400'}`} />
            ...
            {selected ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-500">
                        <span className="flex items-center gap-1"><FiMail /> {selected.email}</span>
                        {selected.phone && <span className="flex items-center gap-1"><FiPhone /> {selected.phone}</span>}
                      </div>
                    </div>
                  </div>
                  <Link to={`/properties/${selected.property?._id}`} className="text-xs text-primary-600 hover:underline flex items-center gap-1">
                    View Property <FiArrowRight />
                  </Link>
                </div>

                <div className="p-4 bg-gray-50 border-b border-gray-100">
                  <div className="flex items-center gap-2 text-xs text-gray-600">
                    <FiHome /> {selected.property?.title} - {selected.property?.location?.city}
                  </div>
                  <span className={`inline-block mt-1 px-2 py-0.5 rounded text-xs font-medium ${
                    selected.status === 'new' ? 'bg-blue-100 text-blue-700' :
                    selected.status === 'read' ? 'bg-gray-100 text-gray-600' :
                    selected.status === 'replied' ? 'bg-emerald-100 text-emerald-700' :
                  ...
                  <form onSubmit={handleReply} className="p-4 border-t border-gray-100">
                    <div className="flex gap-2">
                      <input type="text" value={replyText} onChange={e => setReplyText(e.target.value)}
                        placeholder={isAgent ? "Type your reply..." : "Send a message..."}
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center h-full flex items-center justify-center">
                <div>
                  ...
                  <p className="text-gray-500 text-sm">Choose a message from the list to view the conversation</p>
                </div>
              </div>
            )}
          </div>
